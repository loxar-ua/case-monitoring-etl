...
from src.database.service import get_media, post_article, get_last_published_date, form_clusters
...
class TestServiceCase(TestBaseCase):
    ...
    @patch("src.database.service.get_session")
    def test_form_cluster_relevant(self, mock_get_session):
        """Tests the correct work correctly with relevant cluster"""
        ...
        with patch.object(self.session, 'commit') as mock_commit, \
            ...
            self.assertEqual(retrieved_cluster.featured_image_url, 'image.url')
    ...
    @patch("src.database.service.get_session")
    def test_form_cluster_irrelevant(self, mock_get_session):
        """Tests the correct work correctly with irrelevant cluster"""
        ...
        with patch.object(self.session, 'commit') as mock_commit, \
            ...
            self.assertIn(retrieved_articles[0], retrieved_cluster.articles)
            self.assertIn(retrieved_articles[1], retrieved_cluster.articles)
            self.assertEqual(retrieved_cluster.name, '')
            self.assertEqual(retrieved_cluster.summary, '')
            self.assertEqual(retrieved_cluster.is_relevant, False)
